...
<html lang="en">
	...
	<body onload="ColorFunction()">
		...
		<!--navigation end-->
		<!-- Track Feature-->
		<section class="page-section " style="background-image: url('img/trackBG.jpg'); background-repeat: no-repeat; background-size: cover; background-position: center center; object-fit:cover; " id="contact"   >
			...
			<div class="container justify-content-center text-center text-white" style="height:485px;">
				<h1 class="display-3"> Track your order in real time.</h1>
				<br>
				<h1>Enter your order ID:</h1>
				<center>
				<form id="trackFORM" name="trackFORM" method="post">
					<input placeholder="Ex: A1B5D" class="form-control" style="width:70%;height:50px;font-size:40px;text-align:center;" type="text" id="userSearch" name="userSearch"/>
					<center>
					<p class="text-form-color" id="trackError"></p>
					<br>
					<button class="btn" onclick="ColorFunction()" id="trackSubmit" name="trackSubmit" type="submit">TRACK</button>
				</form>
				<br>
				<h1>The status of your order is:</h1>
				<?php 
					if(isset($_POST["trackSubmit"]))
					{
						$results = mysqli_query($link, "SELECT * FROM tbl_order WHERE userID = '$_POST[userSearch]'");
						
						while($row=mysqli_fetch_assoc($results))
						{
							echo "<div class='display-4 font-weight-bold ' id='statusColor' name='statusColor'>"; echo $row["userStatus"]; echo "</div>";
							echo " <input type='hidden' id='userColor' name='userColor' value='"; echo $row["userStatus"]; echo "'>";
						}
					}
					?>
				</div>
